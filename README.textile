...
Inside each file, it will looks for strings looking like @L["some string"]@ or @L["some string"] = "some other english string"@. In the second case, it will remember the value. The regex used to find these string if pretty rough, so it will likely ignore things like @L[someVar]à and will cause havoc with something like @L["foo"..var.."bar"]@ (which is okay anyway as you must use @string.format(L["foo%sbar"], var)@ for proper localization).
